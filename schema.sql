...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'redemption_status') THEN
    CREATE TYPE redemption_status AS ENUM ('requested', 'approved', 'denied', 'fulfilled');
  END IF;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'redemption_status') THEN
    IF NOT EXISTS (
      ...
      ALTER TYPE redemption_status ADD VALUE 'requested';
    END IF;
    IF NOT EXISTS (
      SELECT 1
      FROM pg_enum
      ...
      ALTER TYPE redemption_status ADD VALUE 'approved';
    END IF;
    IF NOT EXISTS (
      SELECT 1
      FROM pg_enum
      ...
      ALTER TYPE redemption_status ADD VALUE 'denied';
    END IF;
    IF NOT EXISTS (
      SELECT 1
      FROM pg_enum
      ...
      ALTER TYPE redemption_status ADD VALUE 'fulfilled';
    END IF;
  END IF;
END $$;
